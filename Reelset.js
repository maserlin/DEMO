...
Reelset.prototype.spinReels = function(){
    var next = 0;
    var that = this;
    for(var t in GameConfig.getInstance().startTiming){
       setTimeout(function(){
 ...
 * TODO use system timer?
 */
Reelset.prototype.stopReels = function(positions, reelbands){
    var that = this;
    var next = 0;
    for(var t in GameConfig.getInstance().stopTiming){
       setTimeout(function(){
           that.reels[next].stop(positions[next],reelbands[next]);
           ++next 
       },GameConfig.getInstance().stopTiming[t]);      
    }
}
